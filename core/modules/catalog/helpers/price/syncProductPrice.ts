...
export default function syncProductPrice (product, backProduct) { // TODO: we probably need to update the Net prices here as well
  ...
  product.price_incl_tax = typeof backProduct.price_info.final_price ==='string' ? ( parseFloat(backProduct.price_info.final_price) ? parseFloat(backProduct.price_info.final_price) : 0 ): backProduct.price_info.final_price
  ...
  product.price = typeof backProduct.price_info.extension_attributes.tax_adjustments.final_price ==='string' ? ( parseFloat(backProduct.price_info.extension_attributes.tax_adjustments.final_price) ? parseFloat(backProduct.price_info.extension_attributes.tax_adjustments.final_price) : 0 ): backProduct.price_info.extension_attributes.tax_adjustments.final_price
