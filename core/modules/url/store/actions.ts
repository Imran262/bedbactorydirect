...
export const actions: ActionTree<UrlState, any> = {
  ...
  async registerMapping(
    { state },
    { url, routeData }: { url: string; routeData: any }
  ) {
  ...
  async registerDynamicRoutes({ state, dispatch }) {
    ...
    const registrationRoutePromises = Object.keys(state.dispatcherMap).map(
      url => {
  ...
  mapUrl({ state, dispatch }, { url, query }: { url: string; query: string }) {
    ...
    return new Promise((resolve, reject) => {
      if (state.dispatcherMap[url]) {
        return resolve(
          parametrizeRouteData(state.dispatcherMap[url], query, storeCodeInPath)
        );
      }
      cacheStorage
        .getItem(url)
        .then(routeData => {
          if (routeData !== null) {
            return resolve(
              parametrizeRouteData(routeData, query, storeCodeInPath)
            );
          } else {
            const mappingActionName = config.urlModule.enableMapFallbackUrl
              ? "mapFallbackUrl"
              : "mappingFallback";
            dispatch("mappingFallback", { url, params: parsedQuery })
              .then(mappedFallback => {
...
//console.log("1122 cms page is ",cms.length);
  ...
  async getFallbackByUrl(context, { url, params }) {
    const groupId =
      (config.usePriceTiers && context.rootState.user.groupId) || null;
  ...
  async saveFallbackData({ commit }, { _type, _source }) {
    switch (_type) {
      ...
      case "category": {
        commit(
          "category-next/" + categoryMutationTypes.CATEGORY_ADD_CATEGORY,
          _source,
          { root: true }
        );
        break;
      ...
      case "cms_page": {
        commit(
          "cmsPage/" + cmsPageMutationTypes.CMS_PAGE_ADD_CMS_PAGE,
          _source,
          { root: true }
        );
        commit(
          "cmsPage/" + cmsPageMutationTypes.CMS_PAGE_SET_CURRENT,
          _source,
          { root: true }
        );
        break;
  ...
  setCurrentRoute({ commit, state, rootGetters }, { to, from } = {}) {
    ...
    const hasDifferentPath =
      (state.currentRoute && state.currentRoute.path) !== (from && from.path);
