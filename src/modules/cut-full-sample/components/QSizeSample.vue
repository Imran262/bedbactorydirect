<template>
  <button type="button" class="new_button order-sample-q" @click="requestCutSizeSample" :disabled="disabled">
    <span>
      <span class="cut-sample-free" v-if="price !== 0">
        QUARTER SIZE SAMPLE {{ ((price) ? (' - £' + price) : '') }}
      </span>
      <span class="cut-sample-free" v-else>QUARTER SIZE SAMPLE</span>
    </span>
  </button>
</template>
      ...
      await this.$store
        ...
        .then((res) => {
          ...
          this.notifyUser(
            notifications.createNotification({
              type: 'success',
              message: 'Quarter Size Sample updated to cart'
            })
          );
        })
        .catch((err) => {
