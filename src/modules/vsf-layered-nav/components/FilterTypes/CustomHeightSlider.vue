...
export default {
  ...
  mounted() {
    ...
    if (routeQueryData.hasOwnProperty("height")) {
      const routePriceRange = routeQueryData["height"].split("-");
  ...
  computed: {
    ...
    tooltipContent() {
      return { formatter: "{value} " + this.currencySign  };
    },
    getMin() {
  ...
  methods: {
    ...
    validateRoute() {
      ...
      if (this.$refs.priceSlider && !routeQueryData.hasOwnProperty("height")) {
        this.$nextTick(() => {
          this.$refs.priceSlider.setValue(this.priceRange);
        });
      }
