    ...
    <div class="modal-content">
      <div class="scroll">
          ...
          <div class="row m0 firstheading">
            ...
            <div class="col-md-8 col-xs-8">
              <ul
                ...
                "
              >
                <!-- TODO: Will Implemented Later, if requested, else will remove this commented out code-->
                <!--                <template v-if="product.type_id === 'bundle'">
                  <li>
                    <dl>
                      <template v-for="(bundleItem, index) in selectedBundleOptions">
                        <dt :key="`dt`+index">{{ bundleItem.title }}</dt>
                        <dd :key="`dd`+index" v-html="bundleItem.selected_product" />
                      </template>
                    </dl>
                  </li>
                </template>-->
                ...
                <template v-else>
                  ...
                  <li v-if="product.size">
                    <span class="Qty0">Size:</span> {{ product.size }}
                  </li>
                  <li v-if="product.qty_per_sqm">
                    <div
                      v-if="
                  ...
                  <li v-else>
                    ...
                    <!--                    <span v-if="product.qty_per_sqm" class="square"> m<sup>2</sup></span>-->
                  </li>
                  <li
                    v-if="parseFloat(productSqmQuantity) === 1 &&
                      vinylProductPrice === 0 &&
                      productTileTypeUnitCheck(product) === 'm<sup>2</sup>' &&
                  ...
                  <li
                    class="product_price"
                    v-else-if="
                      vinylProductPrice > 0 && product.filter_type === 'Pack'
                    "
                  >
                    {{ vinylRecommendedItemPrice }}
                  </li>
                  <li class="product_price" v-else>
                    {{
        ...
        <div
          ...
          "
        >
          <h1>Need Adhesive &amp; Grouts for these tiles?</h1>
          <div class="col-md-12 flex firstrow" :class="colWidth ? 'row' : ''">
            <div class="select1" :class="colWidth ? 'col-sm-12 col-xs-12' : ''">
              <span class="dropdown-txt">Fixing to the:</span>
              <select
                class="fixing"
            ...
            <div class="select2" :class="colWidth ? 'col-sm-12 col-xs-12' : ''">
              <span class="dropdown-txt">Made of:</span>
              <select
                class="made"
            ...
            <div class="select3" :class="colWidth ? 'col-sm-12 col-xs-12' : ''">
              <span class="dropdown-txt">with a grout joint of:</span>
              <select
                class="joint"
                v-model="selectGroutJoint"
                @change="onChange($event)"
              >
                <option
                  v-for="(joint, index) in groutJoint"
                  :key="index"
                  :value="joint.value"
                >
                  {{ joint.label }}
          ...
          <div class="row m0 paddingleft">
            <div class="col-md-12">
              <div class="row m0">
                <div class="col-md-12">
                  <span class="note">
                    Please note - Suggested quantities below are based on your
                    tile order.
                  </span>
                </div>
              </div>
              ...
              <div
                class="hidden"
                id="crafty_loader_after"
                ref="spinner_one"
              ></div>
              <template
                v-if="
                  cartItemId &&
                  (groutAdhesiveArray[0] || groutAdhesiveArray.length > 0)
                "
              >
                <div v-for="(data, index) in groutAdhesiveArray" :key="index">
                  <RecommendedAdhesives
                    ...
                    :selected-grout-joint="selectGroutJoint"
                    :cart-item-id="cartItemId"
                    v-if="data.type === 'adhesive'"
                    :disabled-check="'No'"
                  />
                  <RecommendedGrouts
                    :data="data"
                    ...
                    :selected-grout-joint="selectGroutJoint"
                    :cart-item-id="cartItemId"
                    v-if="data.type === 'grout'"
                    :disabled-check="'No'"
                  />
                </div>
              </template>
              <div class="row lastrow">
                <div class="col-md-6 col-sm-6 col-xs-6 nothankscol">
                  <button class="nothanks" @click="hidemodal">
                    {{ $t('No Thanks') }}
                  </button>
                </div>
                <div class="col-md-6 col-sm-6 col-xs-6 addallcol">
                  <button class="Addall" @click="hidemodal">
                    {{ $t('Add all for £79.46') }}
                  </button>
                </div>
              </div>
