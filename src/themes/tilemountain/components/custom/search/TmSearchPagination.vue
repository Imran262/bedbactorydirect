...
export default {
  ...
  methods: {
    setPage (page) {
      ...
      const secondSearchHead = this.$parent.$refs.secondSearchHead;
      // BreadCrumbs Row
      const breadcrumbsClass = document.getElementsByClassName('breadcrumbs')[0]
      let breadcrumbsHeight = 0;
      if (breadcrumbsClass !== undefined && breadcrumbsClass !== 'undefined' ) {
        breadcrumbsHeight = parseFloat(breadcrumbsClass.offsetHeight);
      }
      // Filter Row
      const filterRow = document.getElementsByClassName('SearchText')[0]
      const filterRowHeight = parseFloat(filterRow.offsetHeight);

      const elementPosition = secondSearchHead.offsetTop;
