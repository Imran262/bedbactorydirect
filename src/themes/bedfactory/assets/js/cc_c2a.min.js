...
clickToAddress.prototype.phoneValidateCall = function(phone_number, country_code, get_network_information, success_function) {
    "use strict";
    var that = this;
    var parameters = {
        key: this.key,
...
clickToAddress.prototype.emailValidateCall = function(email_address, success_function) {
    "use strict";
    var that = this;
    var parameters = {
        key: this.key,
...
clickToAddress.prototype.apiRequest = function(action, parameters, callback) {
    ...
    if (action === "find" && (this.excludeAreas.length > 0 || this.excludePoBox)) {
...
clickToAddress.prototype.error = function(code, message) {
    ...
    var errors = {
        ...
        JS508: {
            default_message: "beforePopulate callback function resulted in a JS error.",
            level: 0
        },
        JS509: {
            default_message: "beforeCorrect callback function resulted in a JS error.",
            level: 0
        },
        JS401: {
...
function clickToAddress(config) {
    ...
    that.getAvailableCountries(function() {
        ...
        var isValidCountry = function(country, validCountries) {
            for (var i = 0; i < validCountries.length; i++) {
                if (country.toLowerCase() === validCountries[i].code || country.toLowerCase() === validCountries[i].short_code) {
                    return true
                }
            }
            return false
    ...
    this.tools.ccEvent(that.resultList, "scroll", function() {
        ...
        if (that.searchStatus.inCountryMode != 1 && parseInt(this.scrollHeight) !== 0 && scrollTop + innerHeight >= parseInt(this.scrollHeight) * .8) {
    ...
    if (typeof this.activeDom.line_1 != "undefined") {
        ...
        if (addressData.post_office_reference_1 !== "") {
            if (typeof this.activeDom.post_office_reference_1 != "undefined") {
                this.activeDom.post_office_reference_1.value = addressData.post_office_reference_1
            }
        }
        ...
        if (this.getCfg("limitToMaxLength").enabled) {
            ...
            for (var i = 0; i < fields.length; i++) {
                if (typeof this.activeDom[fields[i]] != "undefined" && this.activeDom[fields[i]].getAttribute("maxlength") !== null && !isNaN(this.activeDom[fields[i]].getAttribute("maxlength"))) {
                    ...
                    if (maxLength < 1) {
                        ...
                        var tempValue = this.activeDom[fields[i]].value.substring(0, maxLength);
                        if (this.getCfg("limitToMaxLength").useEllipsis) {
                            tempValue = tempValue.slice(0, -1).trim() + "…"
                        }
                        if (fields[i] === "company") {
...
clickToAddress.prototype.showResults = function() {
    ...
    for (var i = 0; i < _cs.searchResults.results.length; i++) {
        ...
        var content = "<div>";
        if (typeof labels[0] == "string" && labels[0] !== "") content += "<span>" + labels[0] + "</span>";
        if (typeof labels[1] == "string" && labels[1] !== "") content += '<span class="light">' + labels[1] + "</span>";
        if (typeof row.count == "number" && row.count > 1) content += '<span class="light">' + _cs.texts.more.replace("{{value}}", row.count) + "</span>";
        content += "</div>";
...
clickToAddress.prototype.selectCountry = function(countryCode, skipSearch) {
    ...
    for (var i = 0; i < this.validCountries.length; i++) {
        if (countryCode.toLowerCase() === this.validCountries[i].code || countryCode.toLowerCase() === this.validCountries[i].short_code) {
...
clickToAddress.prototype.addPhoneVerify = function(config) {
    ...
    var validated_config = {
        ...
        allowed_type: ct.valExist(config, "allowed_type") ? config.allowed_type : "all",
        get_network_information: ct.valExist(config, "get_network_information") ? config.get_network_information : false,
        can_correct: ct.valExist(config, "can_correct") ? config.can_correct : true,
        trim_spaces: ct.valExist(config, "trim_spaces") ? config.trim_spaces : false,
        beforeCorrect: config.beforeCorrect ? config.beforeCorrect : null
    };
    var phone_field = ct.getMixedElem(validated_config.phone_field);
    if (["all", "mobile", "landline"].indexOf(validated_config.allowed_type) == -1) {
    ...
    ct.tools.ccEvent(phone_field, "keyup", function(e) {
        var noActionKeys = [9, 27, 37, 38, 39, 40, 33, 34, 35, 36, 42, 44, 45, 16, 17, 18, 19, 20];
        if (noActionKeys.indexOf(e.keyCode) != -1) {
            return
        }
        if (ct.last_phone_validation != phone_field.value) {
            ct.validate_phone_sequence++;
            var current_sequence = ct.validate_phone_sequence;
            ct.last_phone_validation = phone_field.value;
            setTimeout(function() {
...
clickToAddress.prototype.phoneVerifyAction = function(config) {
    ...
    if (phone_number == "" || !phone_number.match(regex_check)) {
    ...
    ct.phoneValidateCall(phone_number, country_code, config.get_network_information, function(result) {
        if (result == "error") {
        ...
        if (result.result) {
            if (!config.can_correct && [trim_phone_number(result.phone_number), trim_phone_number(result.national_format)].indexOf(trim_phone_number(phone_field.value)) == -1) {
                ct.validateCssSwap("fail", config.unique_tag);
                error_object = ct.validation_error(1001);
                if (config.callback) {
                    config.callback(false, error_object)
            ...
            if (config.allowed_type == "all" || typeof result.carrier == "undefined" || config.allowed_type == result.carrier.type) {
                ...
                ct.validateCssSwap("fail", config.unique_tag);
                error_object = ct.validation_error(config.allowed_type == "mobile" ? 1002 : 1003);
                if (config.callback) {
...
clickToAddress.prototype.addEmailVerify = function(config) {
    ...
    ct.tools.ccEvent(email_field, "keyup", function(e) {
        ...
        if (ct.last_email_validation != email_field.value) {
            ct.validate_email_sequence++;
            var current_sequence = ct.validate_email_sequence;
            ct.last_email_validation = email_field.value;
            setTimeout(function() {
...
clickToAddress.prototype.validateCssSwap = function(new_rule, tag) {
    var ct = this;
    var box = document.querySelector('#cc_validate_box[cc_validate_tag="' + tag + '"]');
...
clickToAddress.prototype.verifyReposition = function(target, offset, tag) {
    ...
    box.style.width = height * .7 + "px";
    box.style.height = height * .7 + "px";
    var topOffset = elemRect.top + docTop + height * .15;
    var leftOffset = elemRect.right + docLeft - height * .85;
    if (offset) {
...
clickToAddress.prototype.getUniqueTag = function() {
    ...
    var high = 999999;
    var unique_tag = null;
    while ((unique_tag = Math.floor(Math.random() * (high - low + 1) + low)) && document.querySelector('#cc_validate_box[cc_validate_tag="' + unique_tag + '"]') != null) {}
    return unique_tag
};
...
clickToAddress.prototype.validation_error = function(code) {
    ...
    return {
        code: code,
        text: ct.validation_error_reasons[code.toString()]
    }
};
...
clickToAddress.prototype.setupText = function(textCfg) {
    "use strict";
    this.texts = {
        default_placeholder: "Start typing your address or postcode...",
...
clickToAddress.prototype.setPlaceholder = function(country, target) {
    ...
    if (this.placeholders) {
        if (country) {
            ...
            var overridePlaceholder = this.texts.placeholder_country_overrides[placeholderCountry];
...
clickToAddress.prototype.hide = function(force_it) {
    ...
    if (force_it || this.visible && !this.focused && !this.hover) {
        ...
        this.setPlaceholder(0);
        if (this.activeInput != "init") {
            this.activeInput.className = this.activeInput.className.replace(" c2a_active_s", "").replace(" c2a_active", "");
...
clickToAddress.prototype.attach = function(dom, cfg) {
    ...
    var cfg = cfg || {};
    var domElements = {};
    var objectArray = ["search", "postcode", "town", "line_1", "line_2", "company", "county", "country", "post_office_reference_1"];
    var quickGet = null;
    switch (this.domMode) {
        case "id":
    ...
    if (target && target.getAttribute("cc_applied") == "true") {
