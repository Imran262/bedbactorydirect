<template>
  ...
  >
    ...
    <div class="minicart-checkout" v-if="productsInCart.length && !isCheckoutMode">
      <button-full
        :link="{ name: 'checkout' }"
        @click.native="closeMicrocartExtend"
      >{{ $t('Go to checkout') }}</button-full>
    </div>
    ...
    <div class="row middle-xs bg-cl-primary top-sm px40 actions">
      <div class="col-xs-12 col-sm">
        <h2 v-if="productsInCart.length" class="cl-accent mt35 mb35">{{ $t('Shopping cart') }}</h2>
      </div>
    ...
    <h4
      ...
      class="cl-accent ml30"
    >{{ $t('Your shopping basket is empty.') }}</h4>
    <div v-if="!productsInCart.length" class="ml30" @click="closeMicrocartExtend">
      {{ $t("Don't hesitate and") }}
      <router-link :to="localizedRoute('/')">{{ $t('browse our catalog') }}</router-link>
    ...
    <div v-if="productsInCart.length" class="summary px40 cl-accent serif">
      ...
      <!-- <div class="row py20">
        <div v-if="OnlineOnly && !addCouponPressed" class="col-xs-12">
          <button
            class="p0 brdr-none serif fs-medium cl-accent bg-cl-transparent voucher-code-btn"
            type="button"
        ...
        <div v-if="OnlineOnly && addCouponPressed" class="col-xs-12 pt30 coupon-wrapper">
          <div class="coupon-input">
            <label class="h6 cl-secondary">{{ $t('Enter your coupon if you have one') }}</label>
            <base-input
              type="text"
              id="couponinput"
              :autofocus="true"
      ...
      <div
        ...
        v-if="segment.code === 'grand_total' && segment.title !== 'VAT'"
      >
        <div class="col-xs h4 total-price-label">{{ segment.title }}</div>
        <div class="col-xs align-right h2 total-price-value">{{ segment.value | price }}</div>
      </div>
    </div>

    <div class="row py20 px40 middle-xs actions" v-if="productsInCart.length && !isCheckoutMode">
...
/*Cart Apply coupon*/
button.voucher-code-btn {
  padding: 20px 10px;
  background-color: #333;
  min-width: 250px;
  color: #fff;
}
button.voucher-code-btn:hover {
